...
func (m *mockObserver) WillUnloadBranchNode(hex []byte, hash libcommon.Hash, incarnation uint64) {
}

func (m *mockObserver) WillUnloadNode(hex []byte, hash libcommon.Hash) {
...
func TestObserverCodeSizeChanged(t *testing.T) {
	...
	for _, key := range keys {
		...
		acc.CodeHash = libcommon.BytesToHash(codeHash2)
		trie.UpdateAccount(key, acc)
...
func TestObserverUnloadStorageNodes(t *testing.T) {
	...
	for i, storageKey := range storageKeys {
		fullKey := dbutils.GenerateCompositeTrieKey(libcommon.BytesToHash(key), libcommon.BytesToHash(storageKey))
		fullKeys[i] = fullKey
	}
