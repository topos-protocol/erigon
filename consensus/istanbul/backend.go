...
type Backend interface {
	...
	// the given validator
	CheckSignature(data []byte, addr libcommon.Address, sig []byte) error

	// LastProposal retrieves latest committed proposal and the address of proposer
	LastProposal() (Proposal, libcommon.Address)

	// HasPropsal checks if the combination of the given hash and height matches any existing blocks
	HasPropsal(hash libcommon.Hash, number *big.Int) bool
	...
	// IsQBFTConsensus chliblibcommonecks qbftBlock fork block and returns if it should be enabled
