...
func (e *Engine) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool, validators consensus.ValidatorSet) (chan<- struct{}, <-chan error) {
...
func (e *Engine) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, validators consensus.ValidatorSet, parents []*types.Header) error {
...
func (e *Engine) verifySigner(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, validators consensus.ValidatorSet) error {
...
func (e *Engine) verifyCommittedSeals(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, validators consensus.ValidatorSet) error {
	number := header.Number.Uint64()
	...
	if validatorContract != (libcommon.Address{}) && e.cfg.GetValidatorSelectionMode(currentBlockNumber) == libchain.ContractMode {
		...
		for _, transition := range e.cfg.Transitions {
			if transition.Block.Cmp(currentBlockNumber) == 0 && len(transition.Validators) > 0 {
				toRemove := make([]consensus.Validator, 0, validators.Size())
				l := validators.List()
...
func (e *Engine) accumulateRewards(chain consensus.ChainHeaderReader, state *state.IntraBlockState, header *types.Header) {
